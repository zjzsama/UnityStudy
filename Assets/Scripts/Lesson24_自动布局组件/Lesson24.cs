...
public class Lesson24 : MonoBehaviour
{
    ...
    {
        #region 知识点一 自动布局是什么
        // 虽然UGUI的RectTransform已经非常方便的帮我们进行快速布局
        // 但UGUI还提供了很多帮我们对UI控件进行自动布局的组件
        // 他们可以帮我们自动的设置UI控件的位置和大小

        // 自动布局的工作方式一般是
        // 自动布局控制组件+布局元素=自动布局

        // 自动布局控制组件:Unity提供了很多用于自动布局的管理性质的组件 用于布局
        ...
        // 2.如果父类容器中有足够的可用空间，则分配Preferred width和 Preferred height
        ...
        // 参数相关:
        // Aspect Mode:适配模式，如果调整矩形大小天实施宽高比
        // None:不让矩形适应宽高比
