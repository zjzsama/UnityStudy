...
public class Lesson1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Math是C#中封装好的用于数学计算的工具类-位于System命名空间下
        Math.Abs(-10);
        // Mathf是Unity中封装好的用于数学计算的工具结构体-位于UnityEngine命名空间下
        Mathf.Abs(-10);
        #endregion

        #region 知识点二 他们的区别
        // 两者相关方法几乎一样
        // Math是C#自带的工具结构类 主要提供一些数学相关计算方法
        // Mathf是Unity专门封装的 不仅包含Math的方法 还多了一些适用于游戏开发的方法
        // 所以我们在进行Unity游戏开发时
        // 使用Mathf即可
        #endregion
        ...
        print(Mathf.CeilToInt(1.00000001f));
        ...
        print(Mathf.Max(10, 20, 30, 40, 50));

        // 7.获取最小值 - Min
        print(Mathf.Min(1, 2, 3));

        // 8.一个数的N次幂 - Pow
        print("一个数的N次幂" + Mathf.Pow(4, 2));
        print("一个数的N次幂" + Mathf.Pow(3, 2));
        ...
        // 11.判断一个数是不是2的n次方 -IsPowerOfTwo
        print("判断一个数是不是2的n次方" + Mathf.IsPowerOfTwo(2));
        print("判断一个数是不是2的n次方" + Mathf.IsPowerOfTwo(3));
        print("判断一个数是不是2的n次方" + Mathf.IsPowerOfTwo(4));

        // 12.判断正负数 -Sign
        print("判断正负数" + Mathf.Sign(-10)); //正数返回1 负数返回-1
