...
public class Lesson7 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        #region 知识点二 安装2D Sprite
        // 新版本Unity需要安装2D Sprite包才能使用SpriteEditor
        #endregion

        #region 知识点三 Single图片编辑 功能讲解
        // Single图片编辑 主要讲解的就是在设置图片时
        // 将精灵图片模式(Sprite Mode)设置为Single的精灵图片在SpriteEditor窗口中如何编辑
        ...
        // 自定义边缘线设置,可以自定义精灵网络的轮廓形状
        // 默认情况下不修改 都是在矩形网格上渲染 边缘外部透明区域会被渲染 浪费性能
        // 使用自定义轮廓 可以调小透明区域 提高性能

        // 3.Custom Physics Shape(决定碰撞判断区域)
        // 自定义精灵图片的物理形状 主要用于设置需要物理碰撞的2D图形
        // 它决定了之后产生碰撞检测的区域

        // 4.Secondary Textures(为图片添加特殊效果)
        // 次要纹理设置 可以将其他纹理和该精灵图片关联
        // 着色器可以得到这些辅助纹理然后用于一些效果处理
        // 让精灵应用其他效果
        #endregion
    }
