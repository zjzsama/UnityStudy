...
public class Lesson39 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        #region 知识点四 代码换装
        // 1.获取各部位的SpriteResolver(需要引入命名空间)
        // 2.使用SpriteResolver的API进行装备切换
        // sr.GetCategory() 获取当前部位默认的类别名
        // sr.SetCategoryAndLabel() 设置当前部位想要切换的图片信息
        // sr.SetCategoryAndLabel(sr.GetCategory(), "CASK 3");
        SpriteResolver[] srs = this.GetComponentsInChildren<SpriteResolver>();
        foreach (SpriteResolver resolver in srs)
        {
            equipDic.Add(resolver.GetCategory(), resolver);
        }
        ChangeEquip("CASK", "CASK 3");
        #endregion
    }
